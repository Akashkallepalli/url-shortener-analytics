...
def create_short_url(payload: ShortenRequest, db=Depends(get_db)):
    ...
    Response: {"short_code": "aB7xK9m", "original_url": "..."}
    """
...
def redirect_short_code(short_code: str, request: Request, db=Depends(get_db)):
    """
    Redirect to original URL (HTTP 302) and record click analytics.
    """
    url = db.query(UrlMapping).filter(UrlMapping.short_code == short_code).first()
    if not url:
...
def get_stats(short_code: str, db=Depends(get_db)):
    ...
    short_code, original_url, total_clicks, created_at.
    """
    url = db.query(UrlMapping).filter(UrlMapping.short_code == short_code).first()
    if not url:
        raise HTTPException(status_code=404, detail="Short code not found")
